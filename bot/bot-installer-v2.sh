...
cat > /etc/jvo-script/jvo/var.txt << END
BOT_TOKEN="$bottoken"
ADMIN="$admin"
DOMAIN="$domain"
PUB="$PUB"
...
echo -e '#!/bin/bash\ncd /etc/jvo-script/\npython3 -m jvo' > /usr/bin/bot-jvo
chmod 777 /usr/bin/bot-jvo
...
rm -rf /root/bot-installer-v2.sh
clear
neofetch
